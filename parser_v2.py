...
def main():
    ...
    while True:
        ...
        # В связи с возникновением ошибки при считывании файла template_v2.html, в код были внесены правки
        # Была испоьзована встроенная библиотека codecs
        # С ее использованием программа заработала снова
        # Ссылка на ресурс: https://ru.stackoverflow.com/questions/33868/%D0%9E%D1%88%D0%B8%D0%B1%D0%BA%D0%B0-%D0%BA%D0%BE%D0%B4%D0%B8%D1%80%D0%BE%D0%B2%D0%BA%D0%B8-%D0%B2-python-readline-%D0%BF%D1%80%D0%B8-%D1%87%D1%82%D0%B5%D0%BD%D0%B8%D0%B8-utf-8-%D1%84%D0%B0%D0%B9%D0%BB%D0%B0-%D1%80%D1%83%D0%B3%D0%B0%D0%B5%D1%82%D1%81%D1%8F-charmap
        # Текст ошибки, которая возникала до применения библиотеки codecs: UnicodeDecodeError: 'charmap' codec can't decode byte 0x98 in position 8927: character maps to <undefined>
        file = codecs.open("template_v2.html", "r", "utf_8_sig")

        # Также возникло смещение в новых отчетах
        ...
        try:
            ...
            print(
                'Внимание: Возникла ошибка связанная с невозможностью найти элемент. Работы парсера будет продолжена через 5 секунд...')
            time.sleep(5)
            current_page -= 1
            pagination_a = pagination_wrapper.find_element(By.LINK_TEXT, f'{current_page}')
            pagination_a.click()
    ...
    html_content = html_content.replace('{all_products_count}',
                                        f'Общее количество товаров продавца: {total_products_count}')
    ...
    html_content = html_content.replace('{sellers_name}',
                                        f'Продавец: <a href="{url}" target="_blank" rel="noopener noreferrer">{seller_name}</a>')
    ...
    html_content = html_content.replace('{CreatedByIgorVeshkin}',
                                        'ПО формирования отчетов для площадки Plati Market создано Игорем Вешкиным (by IgorVeshkin)')

    with open(f'Reports_result_v2_{seller_name}_{datetime.now().strftime("%d_%m_%Y--%H_%M")}.html', 'w',
              encoding="utf-8") as html_savefile:
